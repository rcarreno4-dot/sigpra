-- Modelo Entidad-Relacion - Practicas Academicas (Version Reducida)
-- Enfoque: gestion de horas con bitacora, evidencia, validacion docente y cierre por directora.
...
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE bitacora CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
...
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE director CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
...
-- Limpieza de tablas fuera de alcance (version anterior)
...
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE plantilla_bitacora CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE hallazgo_mejora CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/

BEGIN EXECUTE IMMEDIATE 'DROP TRIGGER bi_evidencia'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -4080 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TRIGGER bi_bitacora'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -4080 THEN RAISE; END IF; END;
/
...
/
BEGIN EXECUTE IMMEDIATE 'DROP TRIGGER bi_director'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -4080 THEN RAISE; END IF; END;
/
...
/
BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE seq_bitacora'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN RAISE; END IF; END;
/
...
/
BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE seq_director'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN RAISE; END IF; END;
/
...
CREATE TABLE bitacora (
    ...
    horas_reportadas      NUMBER(6,2)    NOT NULL,
    estado_validacion     VARCHAR2(20)   DEFAULT 'PENDIENTE' NOT NULL,
    observacion_docente   VARCHAR2(1000),
    id_docente_validador  NUMBER,
    fecha_validacion      TIMESTAMP,
    fecha_registro        TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL,
    CONSTRAINT pk_bitacora PRIMARY KEY (id_bitacora),
    CONSTRAINT fk_bitacora_practica FOREIGN KEY (id_practica)
        REFERENCES practica(id_practica),
    CONSTRAINT fk_bitacora_docente FOREIGN KEY (id_docente_validador)
        REFERENCES docente_asesor(id_docente),
    CONSTRAINT ck_bitacora_horas CHECK (horas_reportadas > 0),
    CONSTRAINT ck_bitacora_estado CHECK (estado_validacion IN ('PENDIENTE', 'VALIDADA', 'RECHAZADA'))
);
...
CREATE TABLE evidencia (
    ...
    CONSTRAINT fk_evidencia_bitacora FOREIGN KEY (id_bitacora)
...
BEGIN
  IF :NEW.id_director IS NULL THEN
    SELECT seq_director.NEXTVAL INTO :NEW.id_director FROM dual;
  END IF;
END;
/
...
CREATE OR REPLACE TRIGGER bi_bitacora
...
CREATE INDEX ix_practica_periodo ON practica(periodo);

CREATE INDEX ix_bitacora_practica ON bitacora(id_practica);
CREATE INDEX ix_bitacora_estado ON bitacora(estado_validacion);
CREATE INDEX ix_bitacora_fecha ON bitacora(fecha_actividad);

CREATE INDEX ix_evidencia_bitacora ON evidencia(id_bitacora);
